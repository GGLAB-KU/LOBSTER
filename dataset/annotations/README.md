...
This directory contains the LOBSTER gold-standard human annotations as clean JSONL files with only final, adjudicated labels.

> [!IMPORTANT]
> To protect reviewer privacy and prevent these files from appearing in web searches, the `.jsonl` files have been compressed into the password-protected archive `dataset/annotations.zip`.
> **Password**: `lobster`
> 
...
Each line is one annotated review segment (534 total, 529 after excluding *Needs Context*).
...
**Contribution types:** `Modeling`, `NLPApplications`, `DataAndBenchmarking`, `EmpiricalAnalysis`, `LinguisticStudy`, `DomainAdaptation`, `SurveyOrPosition`
...
See the parent [`README.md`](../../README.md) for the annotation guideline and full project overview.
