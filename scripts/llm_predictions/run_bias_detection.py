...
Uses base_runner for shared infrastructure (located at the repository root).
...
# ─── RESPONSE PARSING ─────────────────────
def parse_response(response_text: str) -> tuple[list[dict[str, str]], str]:
    """Parse the LLM JSON response into (biases, parse_error).

    parse_error is "" on success. A failed parse is reported rather than
    returned as an empty list, which would be indistinguishable from a
    review with no bias.
    """
    try:
...
def process_review(
    ...
    try:
        ...
        biases, parse_error = parse_response(response_text)
