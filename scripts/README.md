...
These scripts run inference on the full review corpus. **They require the raw review data** (see [Corpus Compilation](#corpus-compilation)). Shared infrastructure lives in `base_runner.py` at the repository root; the scripts are interactive and will prompt for venue, model and worker count.
...
`run_subcategory_detection.py` classifies the reviews that bias detection flagged
as negative into the four patterns in `prompts/negative_bias_subcategory.md`, so
run it after `run_bias_detection.py` (or after extracting `llm_predictions.zip`):

```bash
python scripts/llm_predictions/run_subcategory_detection.py
```

The released `negative_bias_subcategories/` files were produced before this
script was written, so re-running will not reproduce them byte-for-byte.
...
Prediction scripts read the same `datasets/` directory through `base_runner.py`, so no extra setup is needed once the files are in place.
