...
Claude models (pass an explicit @version for anything not listed):
- claude-opus-4-5
"""
...
class GoogleCloudClient(BaseLLMClient):
    ...
    def _call_claude(
        ...
        # Vertex addresses Claude models as <model>@<version>. Only map names we
        # actually know; guessing a date silently targets a version nobody chose.
        if "@" not in model_name:
            ...
            if model_name not in model_versions:
                raise ValueError(
                    f"Unknown Claude model {model_name!r}. Vertex needs an explicit "
                    f"version, e.g. GOOGLE_CLOUD_MODEL={model_name}@YYYYMMDD. "
                    f"Known shorthands: {sorted(model_versions)}"
                )
